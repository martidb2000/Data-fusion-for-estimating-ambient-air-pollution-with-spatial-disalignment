...
coord_pop=as.matrix(populations_area[,5:6])
...
ggplot()+
  geom_tile(aes(x=G[,1],y=G[,2],fill=y_new))+
  scale_fill_gradient2(low = "green",mid="white",high = "red")+
  geom_point(aes(x=s1[,1],y=s1[,2],col=y,size=y_size))
...
#for(k in 1:(length(yp)-1)){
 # rgl::lines3d(x=sp[k,],y=sp[k+1,],z=ypval)
#}
